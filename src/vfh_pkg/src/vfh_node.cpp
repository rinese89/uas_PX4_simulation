...
: Node("vfh_node")
{
    ...
    m_cell_size       = this->declare_parameter("m_cell_size", 0.1);
    ...
    RCLCPP_INFO(this->get_logger(), "Starting VFH drone node");
    RCLCPP_INFO(this->get_logger(), "pub_px4_setpoint=%s",
...
{
    if (!wandering_mode) {
        goal_pose_world_ = *goal_pose_msg;

        if (goal_pose_world_.header.frame_id.empty()) {
            goal_pose_world_.header.frame_id = "map";
        }
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Got goal pose in frame '%s'. Desired Distance: %.3f; Desired Angle: %.3f",
                    goal_pose_world_.header.frame_id.c_str(),
...
void VFH_node::doTransform()
{
    ...
    const std::string target_frame = goal_pose_world_.header.frame_id.empty() ? "map" : goal_pose_world_.header.frame_id;

    try {
        geometry_msgs::msg::TransformStamped transform =
            tf_buffer->lookupTransform(source_frame, target_frame, rclcpp::Time(0));

        geometry_msgs::msg::PoseStamped goal_in_base;
        tf2::doTransform(goal_pose_world_, goal_in_base, transform);

        p_out = goal_in_base.pose.position;
        ...
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "We could not obtain the transform for goal: %s", ex.what());
    }
}
...
void VFH_node::odomCallback(const nav_msgs::msg::Odometry::SharedPtr odom_msg)
{
    ...
    robot_linear_vel = static_cast<float>(std::hypot(vx, vy));
...
void VFH_node::update()
{
    ...
    if (m_vfh->selectDirection() && pub_px4_setpoint_) {
